...
% [d1,t1] = min(vecnorm(TV-[1.64995 .692387 0],2,2));
% [d2,t2] = min(vecnorm(TV-[1.66394 .680591 0],2,2));
% [d3,t3] = min(vecnorm(TV-[1.66816 .647281 0],2,2));
% [d4,t4] = min(vecnorm(TV-[1.65821 .62619 0],2,2));
t1=2988   ;    t2=2987  ;      t3=2986 ;       t4=2927;
t1234 = [t1 t2 t3 t4];
...
double(diff(Al,alpha))];
adiff = dot(dAda, [E1234c Ehlc])/sum(datas{1}.triangleAreas);
...
xlim([1.5652       1.7315]); ylim([0.59033      0.72146]);
i=1;
    data=datas{i};
