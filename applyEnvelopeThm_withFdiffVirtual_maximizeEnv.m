...
%     xl = xperturb; % use perturbation frames
    ...
    % get xl that minimizes env grad: dLda
...
%     xl = xperturb;
    ...
    extraIn.v3=v3;
    extraIn.Eic=Eic;
    [~, ~, extraOut] = dLda_part(dAda, @(x)obfun_wrapper(x, 1), [l1u l1v l2u l2v], ehat, xl, extraIn);

    adiff_part1(j) = extraOut.val1;
    adiff_part2(j) = extraOut.val2;
...
%{
% this viz is not that reliable. use the scratchspace viz instead.
[~,perm] = sort(adiffs);
figure; 
clf; 
